		...
		if (this.props.print === true) {
			return (
				<div style={{ ...this.props.style }} className={baseClasses}>
					<Form.Group className="form-group mb-3">
						<ComponentLabel {...this.props} name={props.name} htmlFor={props.name} />
						<div>{this.props.data.options.filter((selectedOption) => selectedOption.value === (this.props.value ?? this.props.defaultValue))?.label}</div>
					</Form.Group>
				</div>
			);
		}
...
class Signature extends React.Component {
	...
	render() {
		...
		if (this.state.value && this.state.value.length > 0) {
			sourceDataURL = `data:image/png;base64,${this.state.value}`;
		}
		...
		if (this.props.print === true && !!sourceDataURL) {
			return (
				<div style={{ ...this.props.style }} className={baseClasses}>
					<Form.Group className="form-group mb-3">
						<ComponentLabel {...this.props} name={props.name} htmlFor={props.name} />
						<div><ImageComponent src={sourceDataURL} /></div>
					</Form.Group>
				</div>
			);
		}
		...
		return (
			<div style={{ ...this.props.style }} className={baseClasses}>
				<ComponentHeader {...this.props} name={props.name} htmlFor={props.name} />
				<Form.Group className="form-group mb-3">
...
class Tags extends React.Component {
	...
	getDefaultValue(defaultValue, options) {
		if (defaultValue) {
			return options.filter((option) => defaultValue.indexOf(option.value) > -1);
		}
		return [];
	}
	...
	handleChange = (selected) => {
		if (selected !== undefined && selected !== null && selected.length > 0) {
			this.setState({ value: selected.map((i) => i.value) });

			if (this.props.onChange) {
				this.props.onChange(selected.map((i) => i.value));
			}
		} else {
			this.setState({ value: [] });
		...
		if (this.props.print === true) {
			return (
				<div style={{ ...this.props.style }} className={baseClasses}>
					<Form.Group className="form-group mb-3">
						<ComponentLabel {...this.props} name={props.name} htmlFor={props.name} />
						<div>{this.props.data.options.filter((option) => this.checkedValues.includes(option.value)).map((option) => option.label).join(', ')}</div>
					</Form.Group>
				</div>
			);
		}
		...
		if (this.props.print === true) {
			return (
				<div style={{ ...this.props.style }} className={baseClasses}>
					<Form.Group className="form-group mb-3">
						<ComponentLabel {...this.props} name={props.name} htmlFor={props.name} />
						<div>{this.props.data.options.filter((option) => this.props.value === option.value).map((option) => option.label).join(', ')}</div>
					</Form.Group>
				</div>
			);
		}
